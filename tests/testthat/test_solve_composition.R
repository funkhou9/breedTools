...
test_that("solve_composition with ped option computes expected column means", {
  expect_equal(unname(round(colMeans(test_w_ped)["Duroc"], 5)), 0.04142)
  expect_equal(unname(round(colMeans(test_w_ped)["Hampshire"], 5)), 0.00399)
  expect_equal(unname(round(colMeans(test_w_ped)["Landrace"], 5)), 0.02582)
  expect_equal(unname(round(colMeans(test_w_ped)["Yorkshire"], 5)), 0.92878)
  expect_equal(unname(round(colMeans(test_w_ped)["R2"], 5)), 0.27156)
})
